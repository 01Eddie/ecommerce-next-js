...
export default function ProductTile({ item }) {
  ...
  return (
    <div  onClick={()=> router.push(`/product/${item._id}`)}>
      ...
      {item.onSale === "yes" ? (
        <div className="absolute top-0 m-2 rounded-full bg-red-500">
      ...
      <div className="my-4 mx-auto flex w-10/12 flex-col items-start justify-between">
        <div className="mb-2 flex">
          ...
          >{`S/. ${item.price}`}</p>
          {item.onSale === "yes" ? (
            <p className="mr-3 text-sm font-semibold text-red-700">{`S/. ${(
              item.price -
              item.price * (item.priceDrop / 100)
            ).toFixed(2)}`}</p>
          ) : null}
          {item.onSale === "yes" ? (
            <p className="mr-3 text-sm font-semibold">{`-(${item.priceDrop}%) de descuento`}</p>
          ) : null}
        </div>
